...
export interface ValidateClientImportOptions {
  ...
  sourceSystem: string;
  existingClients?: ExistingClientLookup[];
}
...
async function loadExistingClients(db: TherassistantSupabaseClient): Promise<ExistingClientLookup[]> {
  const { data, error } = await db
    .from("clients")
    .select("id, tenant_id, organization_id, first_name, last_name, date_of_birth, email, phone, external_client_ref")
    .is("archived_at", null)
    .limit(50000);
...
export async function validateClientImportRows(
  ...
  const existingClients = options.existingClients ?? await loadExistingClients(db);

  const bySourceRef = new Map<string, string>();
  const byNameDob = new Map<string, string>();

  for (const client of existingClients) {
    if (options.tenantId && client.tenant_id !== options.tenantId) {
      continue;
    }
